...
function ConversationDetails({ conversationId, mostRecentConversation }) {
  ...
  useEffect(() => {
    const userMessagesRef = ref(db, `userMessages/${conversationId}`);
    onValue(userMessagesRef, (snapshot) => {
      ...
      for (const message in data) {
        if (data.hasOwnProperty(message)) {
          userMessagesArray.push({ id: message, ...data[message], AI: 'User' });
        }
      }
  
      setMessages((prevMessages) => {
        return [...prevMessages, ...userMessagesArray].sort((a, b) => a.timestamp - b.timestamp);
      });
    });
  }, [conversationId]);
      ...
      messages.forEach((message) => {
        setTimeout(() => {
          ...
          if (messageElement) {
           ...
           messageElement.scrollIntoView({ behavior: 'smooth' });    // show the message
          }
        }, delay);
        delay += 200;
      });
  ...
  const handleHashtagClick = (hashtag) => {
    // Perform the filtering based on the clicked hashtag
    // Here you can define the logic to retrieve conversations with the clicked hashtag
    // For example, you can update the state with the filtered conversations
...
return (
    <div>
      ...
      {summary  && (
        <Button variant="contained" onClick={handleToggleMessages}>
